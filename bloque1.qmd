...
Una vez tenemos el dataset cargado, vamos a estudiarlo un poco y a generar una tabla con métricas de cada producto. En particular, vamos a observar qué datos hay **incluidos** en el dataset y nos vamos a quedar con los últimos **4000 productos**.
...
Usaremos la función `prcomp()`de la librería `stats`. También estableceremos los hiperparámetros `scale` y `center` a `TRUE` para estandarizar los datos, ya que PCA es **muy sensible** a las unidades de medida.
...
1. La cuarta y quinta componente principal tiene una distribución de la aportación de cada variable a su creación **similar a la primera y segunda**. La diferencia es que ahora, esas variables más importantes **apuntan a un sentido diferente**, es decir, una de ellas tiene coeficiente negativo, mientras que la otra tiene coeficiente positivo.
...
Para decidir si elegir 2 o 3 componentes, aplicamos el **Criterio de Kaiser**.

```{r}
eig <- pca_productos$sdev^2
barplot(eig, names.arg = paste0("PC", 1:5))
abline(h = 1, lty = 2)
```

Luego, por el criterio de Kaiser, solo nos quedaríamos con las **dos primeras componentes** (son las que tienen el autovalor mayor que 1).

Podemos observar que la tercera componente se queda muy cerca (0.97), pero **el criterio la excluiría**. Aún así, vamos a tenerla en cuenta también **a la hora de analizarlas**.
...
La segunda componente es más interesante, ya que representa **la verbosidad de las reseñas de un producto**. Para esta componente no importa demasiado la valoración media que tiene el producto, sino que lo que realmente marca la diferencia es **cómo de fácil es leer las opiniones de otros usuarios**, penalizando así las reseñas excesivamente largas. Sería interesante ver cómo se relacionaría el sentimiento promedio de las reseñas con esta variable.
